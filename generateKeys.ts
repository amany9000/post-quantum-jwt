...
(async function test() {
    ...
    const hashSphincs =  bytesToHex(blake2s(keysSphincs.publicKey, {dkLen: 16}));
    ...
    writeFileSync('./keys.json', JSON.stringify({
        'keys': {
            [AlgoType.DILITHIUM]: {'secretKey': skHex, 'publicKey': pkHex, 'id': hash},
            [AlgoType.SPHINCS]: {'secretKey': skHexSphincs, 'publicKey': pkHexSphincs, 'id': hashSphincs}
        },
        'ids': {
            [hash] : pkHex ,
